...
namespace Elephant.Hank.Api.Controllers
{
    ...
    public class SchedulerController : BaseApiController
    {
        ...
        public IHttpActionResult ForceExecute(long schedulerId)
        {
            ...
            try
            {
                result = this.schedulerService.ForceExecute(this.UserId, schedulerId, null, null);
            }
            catch (Exception ex)
            {
                this.LoggerService.LogException(ex);
        ...
        /// </summary>
        /// <param name="schedulerId">The scheduler identifier.</param>
        /// <param name="targetToTest">The target to test.</param>
        /// <returns>
        /// Group name of batch
        /// </returns>
        [HttpGet]
        [Route("{schedulerId}/force-execute/target")]
        [CustomAuthorize(ActionType = ActionTypes.Execute, ModuleType = FrameworkModules.Scheduler)]
        public IHttpActionResult ForceExecuteNow(long schedulerId, string targetToTest)
        {
            var result = new ResultMessage<string>();
            try
            {
                var resultDto = this.schedulerService.ForceExecute(this.UserId, schedulerId, targetToTest, null);
        ...
        public async Task<IHttpActionResult> ForceExecuteExternal(long schedulerId)
        {
            ...
            try
            {
                ...
                result = this.schedulerService.ForceExecute(this.UserId, schedulerId, null, requestContent);
            }
            catch (Exception ex)
            {
                this.LoggerService.LogException(ex);
