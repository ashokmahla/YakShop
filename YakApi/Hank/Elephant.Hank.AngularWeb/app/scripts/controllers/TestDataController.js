  ...
  function ($scope, $filter, $rootScope, $q, $stateParams, $state, crudService, ngAppSettings, commonUi, dataProvider) {
      ...
      $scope.loadDataForEdit().then(function () {
        switch ($scope.TestData.LinkTestType) {
          ...
          case 1:
          {
            ...
            for (var i = 0; i < $scope.TestData.SharedTestDataList.length; i++) {
              var lnkSharedStep = _.filter($scope.TestData.SharedTestSteps, {'SharedTestDataId': $scope.TestData.SharedTestDataList[i].Id})[0];
        ...
        for (var i = 0; i < $scope.TestDataList.length; i++) {
          ...
          if ($scope.TestDataList[i].LinkTestType == ngAppSettings.StepTypes.SharedTestStep) {
            ...
            for (var j = 0; j < $scope.TestDataList[i].SharedTest.SharedTestDataList.length; j++) {
              ...
              var lnkSharedStep = _.filter($scope.TestDataList[i].SharedTestSteps, {'SharedTestDataId': $scope.TestDataList[i].SharedTest.SharedTestDataList[j].Id})[0];
    ...
    $scope.onStepTypeChange = function () {
      ...
      switch (LinkTestType) {
        ...
        {
          $scope.resetAllInputControlDisplayStatus();
          if($scope.SharedTestList.length <= 0){
    ...
    $scope.onRequestTypeChange = function () {
      var request = _.filter($scope.RequestTypeList, {'Id': $scope.TestData.ApiTestData.RequestTypeId});
