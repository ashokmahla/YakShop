...
app.controller('ScreenShotCompareController', ['$scope', '$q', '$filter', '$stateParams', '$state', 'CrudService', 'ngAppSettings', 'CommonUiService', 'CommonDataProvider',
  function ($scope, $q, $filter, $stateParams, $state, crudService, ngAppSettings, commonUi, dataProvider) {
    ...
    $scope.onGroupNameFirstChange = function () {
        crudService.getById(ngAppSettings.ReportByExecutionGroupWhereScreenShotArrayExistUrl.format($stateParams.WebsiteId, $scope.CompareData.GroupNameFirst)).then(function (response) {
    ...
    $scope.onGroupNameSecondChange = function () {
      crudService.getById(ngAppSettings.ReportByExecutionGroupWhereScreenShotArrayExistUrl.format($stateParams.WebsiteId, $scope.CompareData.GroupNameSecond)).then(function (response) {
