...
var InputHelper = function () {
    ...
    var TableDataReaderHelper = require('./TableDataReaderHelper.js');
    var tableDataReaderHelper = new TableDataReaderHelper();
        ...
        if (key != null) {
            ...
            if (testInstance.VariableName.trim() == ''
                ...
                switch (testInstance.Action) {
                        ...
                        if (TakeScreenShot) {
                            if (TakeScreenShotBrowser != null && TakeScreenShotBrowser != undefined) {
                                ...
                                if (TakeScreenShotBrowser.ConfigName.toUpperCase() == 'CHROME' && TakeScreenShotBrowser.Platform.toUpperCase() == 'WINDOWS') {
                                    TakeScreenShotBrowser.Platform = "XP"; // Windows NT
                                }
                                //*special handling due to web driver bug*//*
                                                ...
                                                if (Url != UrlBeforeClick) {
                                                    browser.takeScreenshot().then(function (png) {
                                                        browser.getCapabilities().then(function (capabilities) {
                                                            console.log(capabilities.get('platform').toLowerCase());
                    ...
                    case actionConstant.GW9MenuClick:
                    {
                        browser.actions()
                            .mouseMove(key)
                            .mouseMove({x: 30, y: 2})
    ...
    this.GetText = function (key, onSuccess, isSetVar) {
        key.getTagName().then(function (tagName) {
            key.getAttribute("hank-data-table").then(function (isHankDataTable) {
                key.getAttribute("class").then(function (elementClassName) {
                    key.getAttribute("hank-data-table-format").then(function (tableFormat) {
                        ...
                        else if (elementClassName && (elementClassName+"").indexOf("x-grid") >= 0 && isSetVar) {
                            tableDataReaderHelper.readTableFromPcBcCCTable(key, onSuccess);
                        }
                        else if (tagName == "table" && isSetVar) {
                            tableDataReaderHelper.readTableFromTable(key, onSuccess);
                        }
                        else if (tagName == "div" && isHankDataTable == 'true' && isSetVar) {
                            tableDataReaderHelper.readHankCssTable(key, onSuccess);
                        }
                        else {
